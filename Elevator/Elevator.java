...
public class Elevator {
    ...
    // *The currentFloorDestinations array -> (keeps track of the floors the
    // elevator will visit
    ...
    public void callElevator(int start, int destination) {
        ...
        if (requestedPathsMap.containsKey(start)) {
            ...
            // Else add the new key as START with the list containing our DESTINATION
    ...
    private void moveElevator() throws InterruptedException {
        ...
        if (!Arrays.asList(currentFloorDestinations).contains(true) &&
                requestedPathsMap.isEmpty()) {// STOPPING THE ELEVATOR :checking destinations are reached and request
                                              // list is empty
